...
void Installwizard::forceUnmount(const QString &mountPoint) {
  ...
  process.waitForFinished();
  if (process.exitCode() != 0) {
    //  QMessageBox::critical(nullptr, "Error", "Failed to unmount " +
    //  mountPoint);
  } else {
...
void Installwizard::mountPartitions(const QString &drive) {
  QProcess process;
  QString suffix =
      (drive.startsWith("nvme") || drive.startsWith("mmc")) ? "p" : "";
  QString bootPart = QString("/dev/%1%2%3").arg(drive, suffix, "1");
  QString rootPart = QString("/dev/%1%2%3").arg(drive, suffix, "2");

  // 1. Mount root
  process.start("/bin/bash",
                {"-c", QString("sudo mount %1 /mnt").arg(rootPart)});
  process.waitForFinished(-1);

  // 2. Ensure /mnt/boot exists and mount boot
  process.start("/bin/bash", {"-c", "sudo mkdir -p /mnt/boot"});
  process.waitForFinished(-1);
  // 3. Copy ISO for later installation

  process.start("/bin/bash",
                {"-c", QString("sudo mount %1 /mnt/boot").arg(bootPart)});
  process.waitForFinished(-1);

  process.start("/bin/bash",
                {"-c", "sudo cp /tmp/archlinux.iso /mnt/archlinux.iso"});
  process.waitForFinished(-1);
    if (drive.isEmpty())
        return;
...
void Installwizard::mountISO() {
  ...
  if (!QFile::exists(isoPath)) {
    QMessageBox::critical(nullptr, "Error",
                          "Arch Linux ISO not found at: " + isoPath);
    return;
  }
  ...
  QString mountCommand =
      QString("sudo mount -o loop %1 /mnt/archiso").arg(isoPath);
  ...
  if (process.exitCode() != 0) {
    QMessageBox::critical(nullptr, "Error",
                          QString("Failed to mount Arch ISO:\n%1").arg(errors));
    return;
  }

  QMessageBox::information(nullptr, "Success",
                           "Arch Linux ISO mounted successfully,\nand "
                           "dependencies installed\nWill start extracting...");

  // ✅ Step 2: Extract airootfs.sfs to /mnt/rootfs
  QString squashfsPath =
      "/mnt/archiso/arch/x86_64/airootfs.sfs"; // Adjust if necessary

  QString extractCommand =
      QString("sudo unsquashfs -f -d /mnt %1").arg(squashfsPath);
  ...
  if (process.exitCode() != 0) {
    QMessageBox::critical(
        nullptr, "Error",
        QString("Failed to extract Arch Linux root filesystem:\n%1")
            .arg(errors));
    return;
  }
  ...
  QMessageBox::information(
      ...
      "keys and base system.\nThis could take a few...");

  process.start("/bin/bash", QStringList()
                                 << "-c" << " sudo umount -Rfl /mnt/archiso");
...
void Installwizard::installArchBase(const QString &selectedDrive) {
  ...
  if (!QFile::exists("/mnt/usr/bin/pacman")) {
    QString bootstrapUrl = "https://mirrors.edge.kernel.org/archlinux/iso/"
                           "latest/archlinux-bootstrap-x86_64.tar.gz";
    int dlRet = QProcess::execute(
        "sudo", {"wget", "-O", "/tmp/arch-bootstrap.tar.gz", bootstrapUrl});
    if (dlRet != 0) {
      QMessageBox::critical(this, "Error", "Failed to download bootstrap.");
      return;
    }
    int extractRet =
        QProcess::execute("sudo", {"tar", "-xzf", "/tmp/arch-bootstrap.tar.gz",
  ...
  QProcess dnsTest;
  dnsTest.start("sudo",
                {"arch-chroot", "/mnt", "ping", "-c", "1", "archlinux.org"});
  ...
  QProcess::execute("sudo", {"arch-chroot", "/mnt", "pacman", "-Sy",
  ...
  baseProc.start("sudo", {"arch-chroot", "/mnt", "pacman", "-Sy", "--noconfirm",
  ...
  QProcess::execute(
      "sudo", {"cp", tempPresetPath, "/mnt/etc/mkinitcpio.d/linux.preset"});

  QProcess::execute("sudo", {"arch-chroot", "/mnt", "systemctl", "enable",
                             "systemd-timesyncd.service"});

  // Remove rogue archiso config
  QProcess::execute("sudo", {"arch-chroot", "/mnt", "rm", "-f",
                             "/etc/mkinitcpio.conf.d/archiso.conf"});
  QProcess::execute("sudo", {"arch-chroot", "/mnt", "sed", "-i",
                             "s/archiso[^ ]* *//g", "/etc/mkinitcpio.conf"});

  // ⚠️ Remove old initramfs to ensure fresh build
  QProcess::execute(
      "sudo", {"arch-chroot", "/mnt", "rm", "-f", "/boot/initramfs-linux*"});
  appendLog("Regenerating /etc/fstab cleanly…");

  int fstabRet = QProcess::execute("sudo", {"arch-chroot", "/mnt", "bash", "-c",
  ...
  // Set hostname
  QProcess::execute("sudo", {"arch-chroot", "/mnt", "bash", "-c",
                             "echo archlinux > /etc/hostname"});

  // Set locale
  QProcess::execute("sudo",
                    {"arch-chroot", "/mnt", "sed", "-i",
                     "s/^#en_US.UTF-8/en_US.UTF-8/", "/etc/locale.gen"});
  QProcess::execute("sudo", {"arch-chroot", "/mnt", "locale-gen"});
  QProcess::execute("sudo", {"arch-chroot", "/mnt", "bash", "-c",
                             "echo LANG=en_US.UTF-8 > /etc/locale.conf"});

  // Set timezone
  QProcess::execute("sudo", {"arch-chroot", "/mnt", "ln", "-sf",
  ...
  QMessageBox::information(
      ...
      "installation and updating next...");
...
void Installwizard::installGrub(const QString &drive) {
  ...
  int pkgRet = QProcess::execute("sudo", {"arch-chroot", "/mnt", "pacman",
                                          "-Sy", "--noconfirm", "grub",
                                          "os-prober", "--needed"});
  if (pkgRet != 0) {
    QMessageBox::critical(nullptr, "Error",
                          "Failed to install grub+os-prober inside chroot.");
    return;
  }

  // Clean GRUB default file
  QProcess::execute("sudo", {"arch-chroot", "/mnt", "sed", "-i",
                             "/2025-05-01-10-09-37-00/d", "/etc/default/grub"});
  QProcess::execute(
      "sudo",
  ...
  if (efiInstall) {
    grubArgs << "--target=x86_64-efi" << "--efi-directory=/boot"
             << "--bootloader-id=GRUB";
  } else {
  ...
  int cfgRet =
      QProcess::execute("sudo", {"arch-chroot", "/mnt", "grub-mkconfig", "-o",
                                 "/boot/grub/grub.cfg"});
  if (cfgRet != 0) {
